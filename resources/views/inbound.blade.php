{{-- resources/views/inbound.blade.php --}}
@php
  /** @var \Illuminate\Support\Collection|\Illuminate\Pagination\LengthAwarePaginator $stokMasuk */
@endphp

{{-- Sidebar tetap pakai punyamu --}}
...
<style>
  ...
  .inb-table{
    flex: 0 87%;
    background:#fff; border:5px solid #e5e7eb; border-radius:20px; padding:10px;
    overflow:hidden; /* supaya radius kelihatan */
  }

  /* Dark mode selaras sidebar (warna saja) */
  html.dark .inb-form,
  html.dark .inb-table{
    background:#1f2937; border-color:#0062ff49; color:#e5e7eb;
  }
  ...
  .form-input, .form-file, .form-number, .form-date, .form-time{
    width:100%; padding:8px 10px; border-radius:10px;
    border:1px solid #e5e7eb; background:#fff; color:#111827; font-size:14px; outline:none;
  }
  .form-input:focus, .form-file:focus, .form-number:focus, .form-date:focus, .form-time:focus{
    border-color:#c7d2fe; box-shadow:0 0 0 3px rgba(199,210,254,.45);
  }
  html.dark .form-input, html.dark .form-file, html.dark .form-number, html.dark .form-date, html.dark .form-time{
    background:#0f172a; border-color:#334155; color:#e5e7eb;
  }
  html.dark .form-input:focus, html.dark .form-file:focus, html.dark .form-number:focus,
  html.dark .form-date:focus, html.dark .form-time:focus{
    border-color:#717272; box-shadow:none;
  }
  ...
  /* ====== Alert ====== */
  .alert-ok{
    background:#ecfdf5; border:1px solid #a7f3d0; color:#065f46; border-radius:10px; padding:8px; margin-bottom:12px;
  }
  .alert-err{
    background:#fef2f2; border:1px solid #fecaca; color:#991b1b; border-radius:10px; padding:8px; margin-bottom:12px;
  }
  ...
  /* ====== Tabel (simple & seragam) ====== */
  .tbl{ width:100%; border-collapse:collapse; font-size:14px; }
  .tbl th, .tbl td{
    border:1px solid #e5e7eb; padding:8px; text-align:center; vertical-align:top; color:#111827;
  }
  ...
  .foto-thumb{ width:50px; height:50px; object-fit:cover; border-radius:8px; border:1px solid #e5e7eb; }
  html.dark .foto-thumb{ border-color:#334155; }
</style>

<div class="page">
  <div class="inb-wrap">
    <div class="inb-form">
      <h3 class="inb-title">Inbound</h3>
      ...
      <form method="POST" action="{{ route('inbound.store') }}" enctype="multipart/form-data">
        ...
        <div class="form-row">
          ...
          <input class="form-input" type="text" name="kategori" required>
        </div>
        ...
        <div class="form-row">
          <label>Harga</label>
          <input class="form-number" type="number" step="0.01" min="0" name="harga" required>
        </div>
    ...
    {{-- ====== TABLE (87%) ====== --}}
    <div class="inb-table">
      <h3 class="inb-title">Daftar Stok Masuk</h3>
          ...
          <tbody>
            @forelse($stokMasuk as $row)
             <tr>
               ...
               {{-- tanggal: ambil 10 char biar tanpa jam --}}
               <td>{{ \Illuminate\Support\Str::of($row->tanggal)->limit(10,'') }}</td>
         
               {{-- jam: format hanya HH:mm --}}
               <td>
                 @if(!empty($row->jam))
